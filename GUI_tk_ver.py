...
class Application(Frame):
    ...
    def initVar(self):
        ...
        # PreAudio list of selected candidated song
    ...
    def playSeed(self):
        if sys.platform == 'darwin':
            Popen(["afplay", os.path.join(WavLocation,
                                          'normalized-'+self.seedName+'(inst)' + '_1.wav')])
...
#            print 'Mashed File: '+ maxSeg.name[:maxSeg.name.rfind('(inst')]+maxSeg.name[maxSeg.name.rfind('_'):]+'.pgz'
    ...
    def saveMashuped(self):
        outputFile = './' + \
            "".join(self.seedName.strip().split(' '))+'_mashupped.wav'
        librosa.output.write_wav(
            outputFile, self.mashuppedSig, self.seed[0].sr)
    ...
    def actionElements(self):
        ...
        Button(self, text='-> Load ',
               command=self.load).grid(row=1, column=2, sticky='WE')
        Label(self, text="---------Seed---------").grid(row=2,
                                                        column=2, sticky='WE')
        Button(self, text='Play Seed Song', command=self.playSeed).grid(
            row=3, column=2, sticky='WE')
        Button(self, text='Show Seed Wave', command=self.seedShow).grid(
            row=4, column=2, sticky='WE')
        Label(self, text="---------Mash---------").grid(row=5,
                                                        column=2, sticky='WE')
        Button(self, text='*Mashup*',
               command=self.mashupSong).grid(row=6, column=2, sticky='WE')

        Button(self, text='Play Mashuped Song', command=self.playMashuped).grid(
            row=7, column=2, sticky='WE')
        Button(self, text='Show Mashuped Wave', command=self.showMashuped).grid(
            row=8, column=2, sticky='WE')
        Button(self, text='<- Save Mashuped Song',
               command=self.saveMashuped).grid(row=9, column=2, sticky='WE')
