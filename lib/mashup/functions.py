        ...
        for j in xrange(0, len(seedBeatsIndex)+1, degree):
         #     print j,j/degree
            ...
            if j/degree == 0:
                ...
                diff = float(float(
                    candBeatsIndex[j/degree] - candBeatsIndex[(j/degree)-1]) / float(start-end-1) / float(degree))
                print(diff)
                sig = time_stretching(
                    resultSig[end+1:start+1], vocalSr, float(diff))
...
def overlay(candSig, candSr, seedSig, seedSr, vocalSig, vocalSr):
    ...
    resultSig = beat_matching(candSig, candSr, seedSig,
                              seedSr, vocalSig, vocalSr)
    ...
    for i in xrange(preLength+1):
        if i == preLength:
            ...
            resultSig[i] = preOrderSig[i] + \
                postOrderSig[i-preLength+overlapLength]
        else:
            resultSig[i] = preOrderSig[i]
